...
set :keep_releases, 5
set :application, "ejweb"
set :repository,  "git@github.com:dangluan/ejweb.git"
set :scm, :git
set :branch, :master
...
ssh_options[:keys] = %w(/home/dangluan/.ssh/id_rsa)
...
namespace :deploy do
  ...
  task :restart, :roles => :web do
    unicorn_pid = "#{shared_path}/pids/ejweb.pid"
    run "if [ -f #{unicorn_pid} ] && [ -e /proc/$(cat #{unicorn_pid}) ]; then kill -USR2 \`cat #{unicorn_pid}\`; else cd #{current_path} && bundle exec unicorn -c config/unicorn.rb -D -E #{rails_env}; fi"
  end
  ...
  task :stop, :except => { :no_release => true } do
    run "kill -s QUIT `cat #{shared_path}/pids/ejweb.pid`"
  end  
